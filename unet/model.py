...
class Model:
    ...
    def fit_epoch(self, dataset, n_batch=1, shuffle=False):
        ...
        for batch_idx, (X_batch, y_batch, *rest) in enumerate(DataLoader(dataset, batch_size=n_batch, shuffle=shuffle)):
            ...
            print(f"y_batch values: {y_batch.unique()}")

            # 扩展 y_batch 的维度XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
            y_batch = y_batch.unsqueeze(1)  # 将 y_batch 从 (batch_size, height, width) 转换为 (batch_size, 1, height, width)
            #y_batch = y_batch.repeat(1, 2, 1, 1)
            #print(f"y_batch shape before loss calculation: {y_batch.shape}")
            count = count + 1
            print(count)
            ...
            print(f"y_out values: {y_out.unique()}")
            y_batch = (y_batch > 0).float()  # 将 y_batch 转换为 Float 类型
            print(f"y_batch values: {y_batch.unique()}")
    ...
    def val_epoch(self, dataset, n_batch=1, metric_list=MetricList({})):
        ...
        for batch_idx, (X_batch, y_batch, *rest) in enumerate(DataLoader(dataset, batch_size=n_batch)):
            ...
            X_batch = Variable(X_batch.to(device=self.device))
            y_batch_float = Variable(y_batch.to(device=self.device)).unsqueeze(1).float()  # 用于损失计算
            y_batch_int = (y_batch_float > 0).long()  # 用于度量计算

            y_out = self.net(X_batch)
            print(f"y_out values: {y_out.unique()}")
            print(f"y_batch_float values: {y_batch_float.unique()}")
            print(f"y_batch_int values: {y_batch_int.unique()}")

            training_loss = self.loss(y_out, y_batch_float)  # 使用浮点类型的 y_batch 计算损失
            running_val_loss += training_loss.item()
            metric_list(y_out, y_batch_int)  # 使用整数类型的 y_batch 计算度量

        del X_batch, y_batch_float, y_batch_int
    ...
    def predict_dataset(self, dataset, export_path):
        ...
        for batch_idx, (X_batch, *rest) in enumerate(DataLoader(dataset, batch_size=1)):
            ...
            # 确保只访问存在的通道
            if y_out.shape[1] == 1:
                y_out_int = (y_out[0, 0, :, :] * 255).astype(np.uint8)  # 只有一个通道
            else:
                # 如果有多个通道，这里可能需要其他处理，例如选择特定通道或应用阈值
                y_out_int = (y_out[0, 1, :, :] * 255).astype(np.uint8)  # 选择第二个通道

            #io.imsave(os.path.join(export_path, image_filename), y_out[0, 1, :, :])
            ##y_out_int = (y_out[0,1,:,:]*255).astype(np.unit8)
