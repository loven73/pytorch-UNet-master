...
def classwise_iou(output, gt):
    """
    Args:
        output: torch.Tensor of shape (n_batch, n_classes, images.shape)
        gt: torch.LongTensor of shape (n_batch, images.shape)
    """
    if len(gt.shape) == 3:
        gt = gt.unsqueeze(1)  # 将 gt 从 (batch_size, height, width) 转换为 (batch_size, 1, height, width)

        # output的形状是 (batch_size, classes, height, width)
        # gt的形状是 (batch_size, 1, height, width)
    intersection = output * gt
    union = output + gt - intersection
    classwise_iou = (intersection.sum(dim=(0, 2, 3)).float() + EPSILON) / (union.sum(dim=(0, 2, 3)).float() + EPSILON)
    return classwise_iou.mean()  # 返回所有类别的IoU平均值
